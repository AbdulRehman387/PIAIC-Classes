...
let function1 = (array1: string[], _index: number, _name: string) => {
...
let _name: string = "Hamza";
console.log(function1(array1, 3, _name));

// - Implement a simple shopping cart program using an array. Create functions to add items, remove items, and update quantities using the splice method. Print the cart's contents after each operation
let functionToAdd = (array2: string[], item: string) => {
    array2.splice(array2.length, 0, item);
    return array2;
}
let functionToRemove = (array2: string[], item: string) => {
    array2.splice(array2.indexOf(item), 1);
    return array2;
}
let functionToUpdate = (array2: string[], item: string, itemToAdd: string) => {
...
let functionFactorial = (num: number): number => {
    let fact: number = 1;
    let i: number = 1;
    while (i <= num) {
        fact = fact * i;
...
let functionRemoveNegative = (array3: number[]) => {
...
let array3: number[] = [5, 3, -7, -59,-34, 12, 45]
console.log(functionRemoveNegative(array3));

// - Create a function that takes an array of numbers as parameter. Use a while loop to calculate and return the sum of all the numbers in the array.
let functionAddNumbers = (array4:number[]):number =>{
    let sum=0;
    let i=0;
...
let array7:number[] = [24, 43, 12, 45, 87, 42, 16]
